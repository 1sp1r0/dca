...
public abstract class BaseCrawler implements Crawler{
	...
	public LoginData tryLogin(long userId, long companyId, String username, String password) throws Exception {
		...
		LoginData loginData =this.tryLoginUser(userId,username,password,client);
	...
	public CrawlData crawlAccount(final Account account) throws Exception {
		...
		try {
			...
			// try login
			LoginData loginData = this.tryLoginUser(account.getUserId(), account.getUserName(), account.getUserPwd(), client);
	...
	protected abstract LoginData tryLoginUser(long userId, String username, String password, Object client)
			throws Exception;
