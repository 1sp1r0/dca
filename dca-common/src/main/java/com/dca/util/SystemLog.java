...
public class SystemLog {
	 ...
	 *            字符串，输出的信息内容
	 */
	public void writeToFile(String msg, String dir,String fileName) {

		// 组合日志信息
		StringBuffer buf = new StringBuffer();
		buf.append(msg + "\r\n");
		// 写入日志文件
		writeLogFile(buf.toString(),dir, fileName,true);
	}
	...
	public synchronized String readFile(String dir,String fileName) {
	 ...
	 * @param msg
	 */
	public synchronized void writeLogFile(String msg,String dir, String fileName,boolean isAdd) {
		// 利用月份创建文件夹，利用日期获得日志文件，采用追加的方式写入日志文件
		BufferedWriter bw = null;
		try {
			// 判断主路径是否存在，如果不存在创建路径
			...
			bw = new BufferedWriter(new FileWriter(dir+"/"+ fileName + ".txt", isAdd));
			bw.write(msg);
			bw.flush();
		} catch (Exception e) {
			System.out.println("测试日志文件写入失败！");
