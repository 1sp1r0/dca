...
window.light || function (window, undefined) {
    ...
    var light = {
        ...
        debug: urlParts[2].indexOf("alipay.com") === -1,
        baseDomain: function () {
        ...
        extend: function (deep) {
            ...
            for (var i = start, l = args.length; i < l; i++) {
                ...
                for (var prop in obj) {
                    var item = obj[prop];
                    if (item === o || !obj.hasOwnProperty(prop)) {
                        continue
                    }
                    if (light.isArray(item)) {
                        o[prop] = Array.prototype.concat.call(item)
                    } else {
                        if (deep && item instanceof Object && !light.isFunction(item) && !item.nodeType) {
                            var tmp = o[prop] || {};
        ...
        type: function (obj) {
            return (obj === null || obj === undefined) ? String(obj) : class2type[Object.prototype.toString.call(obj)] || "object"
        },
        has: function (path) {
            if (!path) {
                return false
...
light.log || light.extend({
        ...
        if (window.JSON && JSON.stringify) {
            ...
            if (typeof obj === "object") {
                ...
                for (var prop in root) {
                    if (typeof (value = root[prop]) == "object") {
                        result.push(arguments.callee(value))
                    } else {
                        result.push(prop + "=" + root[prop])
                    }
                }
                return result.join("\n")
            } else {
    ...
    unparam: function (text, splitter, connector) {
        ...
        light.each(text.split(connector || "&"), function (i, item) {
            var pair = item.split(splitter, 2);
            if (!pair[0] || pair.length !== 2) {
                return
            }
            obj[light.decode(pair[0])] = light.decode(pair[1])
        });
        return obj
    },
    trimTag: function (html) {
        if (!html || !document.createElement) {
            return ""
        }
 ...
 */
light.has("client/info") || function (window, light, undefined) {
    var document = window.document,
        navigator = window.navigator,
        location = window.location;
    ...
    var data = {
        ...
        browser: {
            ...
            chrome: /chrome\/(\d+)\.(\d+)\.(\d+)(?:\.([ab\d]+))?/,
            safari: /version\/(\d+)\.(\d+)(?:\.([ab\d]+))? safari\//,
    ...
    var detected = -1,
        notDetected = 0,
        info = {}, has = function (type, name, version) {
            var currentVersion;
            if (!info[type] || !(currentVersion = info[type][name])) {
                return false
            }
            if (!version) {
                return true
            ...
            for (var i = 0, len = Math.max(v.length, currentVersion.length); i < len; i++) {
                v1 = parseInt(v[i], 10) || 0;
                v2 = parseInt(currentVersion[i], 10) || 0;
                if (v1 !== v2) {
                    return v1 < v2
                }
            }
    ...
    if (!info.engine.name && window.ActiveXObject) {
        ...
        if (!info.os.windows && info.hasEngine("trident", 6)) {
 ...
 */
light.has("client/storage") || function (window, light, undefined) {
    var document = window.document,
        navigator = window.navigator,
        location = window.location;
    var userDataId = "__ud",
    ...
    var storage = {
        ...
        set: function (name, value) {
            if (storage.cookie && navigator.cookieEnabled) {
                var sCookie = name + "=" + encodeURIComponent(value);
                if (!storage.cookie.days) {
                    var exp = new Date(new Date().getTime() + storage.cookie.days * 365 * 24 * 60 * 60 * 1000);
        ...
        get: function (name, defaultValue) {
            ...
            if (advanced) {
                ...
                if (node) {
                    try {
                        node.load(userDataId)
                    } catch (e) {}
                    value = node.getAttribute(name)
                }
            } if (!value && storage.cookie && navigator.cookieEnabled) {
                ...
                if (start != -1) {
                    ...
                    value = light.decode(cookie.substring(start, end) || "")
                }
            }
...
this.Tracker || function (c) {
    var o = c.document,
        ...
        j = c.light,
        b, q, s = p.protocol,
        x = s + "//kcart.alipay.com/web/bi.do",
        y = s + "//kcart.alipay.com/web/1.do",
        z = s + "//log.mmstat.com/5.gif";
    e && e.timing ? q = e.timing.navigationStart : c._to && _to.start && (q = _to.start.getTime());
    c.Tracker = b = function () {};
    b.prototype = {
        watch: function () {
            b.click("tracker-watch")
        }
    };
    b.extend = function (a) {
        for (var b = 1, t = arguments.length; b < t; b++)
            for (var c in arguments[b]) arguments[b].hasOwnProperty(c) && (a[c] =
                arguments[b][c]);
        return a
    };
    b.version = "1.0";
    ...
    b.seedName = "seed";
    b.minInterval = 1E3;
    for (var u, m = o.getElementsByTagName("meta"), n = 0, f, A = m.length; n < A; n++)
        if ((f = m[n].getAttribute("name")) && "abtest" == f.toLowerCase()) {
            u = m[n].getAttribute("content");
            break
        }
    b.dispatchEvent = function (a, b, c) {
        a.attachEvent ? a.attachEvent("on" + b, function (b) {
            c.call(a, b)
        }) : a.addEventListener ? a.addEventListener(b, c, !1) : a["on" + b] = function (b) {
            c.call(a, b)
        }
    };
    b.getTarget = function (a) {
        ...
        try {
            if (a &&
                3 === a.nodeType) return a.parentNode
        } catch (b) {}
        return a
    };
    b.send = function (a, c, t) {
        if ("string" !== typeof a || !a) throw Error("Invalid page");
        a = {
            ref: c || "-",
            pg: a || "",
            r: (new Date).getTime(),
        ...
        u && (a.ABTest = u, a.pg += (0 <= a.pg.indexOf("?") ? "&" : "?") + "ABTest=" + u);
        t && j.extend(a, t);
        a = j.param(a);
        if ("file:" != s && !b.debug) {
            var d = new Image(1, 1);
            ...
            d.src = x + "?" + a;
            /\bcna=/.test(document.cookie) || (a = j.param({
                url: y + "?" + a
            }), (new Image(1, 1)).src = z + "?" + a)
        } else j.log("Tracker debug: %s.",
            a)
    };
    if (o && p) {
        var m = o.referrer,
        ...
        c.screen && (d.screen = screen.width + "x" + screen.height, d.sc = screen.colorDepth + "-bit");
        d.utmhn = p.hostname;
        d.rnd = v;
        c.analytic_var && (d.ana = analytic_var);
        j.client && (f = j.client.info, p = (f.os.name || "na") + "/" + (f.os.version || [-1]).join("."), n = (f.browser.name || "na") + "/" + (f.browser.version || [-1]).join("."), f = (f.engine.name || "na") + "/" + (f.engine.version || [-1]).join("."), d._clnt = p + "|" + f + "|" + n);
        b.send(i, m, d);
        m = i;
        d = {};
        if (!parseInt(8 *
            ...
            var g = 0,
                ...
                k = function () {
                    if (!k.invoked) {
                        k.invoked = true;
                        g = (c._to && _to.ready ? _to.ready.getTime() : (new Date).getTime()) - q;
                        if (g > 2E4) {
                            l.invoked = true;
                            d.tm = "-x-";
                            b.send(i, "", d)
                    ...
                    if (!(g > 2E4 || l.invoked)) {
                        l.invoked = true;
                        r = (c._to && _to.end ? _to.end.getTime() : (new Date).getTime()) - q;
                        if (e && e.timing) {
                            g = e.timing.domContentLoadedEventStart - e.timing.navigationStart;
                            r = e.timing.loadEventStart - e.timing.navigationStart
                        }
                        w()
                    }
                }, w = function () {
            ...
            if (q) {
                if (c._to && _to.ready) k();
                else if (c.YAHOO && YAHOO.util && YAHOO.util.Event) YAHOO.util.Event.onDOMReady(k);
                else if (c.jQuery) jQuery(k);
                else if (c.Y && Y.on) Y.on("domready", k);
                else k();
                c._to && _to.end ? l() : (c.setTimeout(l, 16E4), b.dispatchEvent(c, "load", l), b.dispatchEvent(c, "unload", l))
            }
        }
    }
    o && b.dispatchEvent(o, "mousedown", function (a) {
        if ((a = b.getTarget(a)) && a.nodeType) {
            for (; a && "HTML" != a.nodeName && a.getAttribute && !a.getAttribute(b.seedName);) a = a.parentNode;
            if (a && !(1 !== a.nodeType || "HTML" == a.nodeName)) {
                var c, d;
                if ("A" === a.nodeName) {
                    d = a.getAttribute("href", 2) || "";
                    if (d === i || 0 === d.indexOf(i + "#")) d = "";
                    (d = d.match(/[?&]_scType=([^&#]+)/)) && (c = {
                        _scType: d[1]
                    })
                }
                b.click(a.getAttribute(b.seedName), c)
            }
        }
    });
    b.click = function () {
        var a = {}, c = i.split("?").shift(),
...
!window.monitor || (function () {
    if (0 != Math.floor(Math.random() / window.monitor._rate)) {
        return
    }
    var win = window,
        loc = location,
        ...
        MODE = {
            ...
            DEV: 3,
            LOCAL: 4
        }, mode, scriptBase, scriptB, LOG_SERVER, SEND_STATUS = {
            COMPLETE: 0,
            SENDING: 1
    ...
    if (M.S.endsWith(M._loc.hostname, ".alipay.com")) {
        ...
        if (M._loc.hostname == "m.sit.alipay.net") {
            if (mode == MODE.LOCAL) {
            ...
            if (M.S.endsWith(M._loc.hostname, ".sit.alipay.net")) {
                mode = MODE.SIT;
                LOG_SERVER = "http://fmsmng.sit.alipay.net:7788/m.gif";
    ...
    M.url = M._loc.protocol + "//" + M._loc.hostname + (idx < 0 ? M._loc.pathname : M._loc.pathname.substr(0, idx));
    M.res = {
        img: [],
        css: [],
        js: [],
    ...
    M.report = function (data) {
        ...
        if (data.hasOwnProperty("htmlError")) {
            var list = part(data.htmlError, URLLength - encodeURIComponent($JSON.toString(DATA)).length - 150);
    ...
    window.setTimeout(function () {
        try {
            ...
            M.timedSend();
            if (M.client.engine.name == "trident" && M.client.engine.version[0] <= 7) {
                return
            }
            if (mode == MODE.LOCAL) {
