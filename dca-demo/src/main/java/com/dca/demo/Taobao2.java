...
public class Taobao2 {
	...
	public Taobao2(String user, String pwd) {
		this.user = user;
		this.pwd = pwd;
		createCrawlerClient();
	}
	...
	protected void createCrawlerClient() {
		...
		System.setProperty("javax.net.ssl.trustStore", "E:/project/data-crawler-architecture/dca-deamo/jssecacerts2");
		...
		// CrawlerClient.ConstructParams params = new
		// CrawlerClient.ConstructParams(true);
	...
	protected boolean doVerify(String verifyCode) throws Exception {
		...
		// String pageContent = client.postPage(REQUEST_NICK_CHECK_URL,
		// postMap);
		...
		// boolean r = (pageContent.indexOf(avlidContent) > -1) ?
	...
	protected void crawlerData() throws Exception {
		...
		pageContent = webClient.getPage("http://easybuy.jd.com/address/getEasyBuyList.action");
	...
	protected void doGetCookie() throws HttpException, IOException {
		...
		// try {
		// pageContent =
		// client.getPage("https://login.taobao.com/member/login.jhtml");
		// } catch (Exception e) {
	...
	public static void main(String[] args) throws DecodingException {
		...
		Taobao2 engine = new Taobao2("15802489620", "yang1258.");
		try {
			engine.doGetCookie();
			...
			// new File("Verify_Code_Image/verify_" + engine.imguuid +
			// ".jpg").delete();
			if (engine.doVerify("")) {
				engine.crawlerData();
			}
